...
const Switch = () => {
  const { handleTheme, theme } = useUiContext();
  ...
  return (
    <div      
      className="z-50 scale-30 sm:scale-40 w-48 aspect-video rounded-xl has-[:checked]:bg-main bg-[#ebe6ef] border-4 border-[#121331]">
      <div className=" flex h-full w-full px-2 items-center gap-x-2">
        ...
        htmlFor="switch" className="has-[:checked]:scale-x-[-1] w-full h-10 border-4 border-[#121331] rounded cursor-pointer">
          <input               onClick={handelToggleTheme}
 type="checkbox" id="switch" className="hidden" />
